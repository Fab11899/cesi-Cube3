...
const navigation = {
    categories: [
        ...
        },
        {
            name: 'Men',
            featured: [
                { name: 'Casual', href: 'category' },
                { name: 'Boxers', href: 'category' },
                { name: 'Outdoor', href: 'category' },
            ],
            collection: [
                { name: 'Everything', href: 'category' },
                { name: 'Core', href: 'category' },
...
onMounted(async () => {
    try {
        ...
        products.value = response.data.map(product => ({
            id: product.id,
            name: product.name,
            href: 'product/' + product.id, // ou `product.href` si tu veux le garder dynamique
            price: `$${parseFloat(product.price).toFixed(2)}`, // "$32.00"
